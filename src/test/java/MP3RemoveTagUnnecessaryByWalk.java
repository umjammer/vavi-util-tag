...
/**
 * MP3RemoveTagUnnecessaryByWalk. (mp3 remove unnecessary tags by directory)
 *
 * @author <a href="mailto:umjammer@gmail.com">Naohide Sano</a> (nsano)
 * @version 0.00 051225 nsano initial version <br>
 */
public class MP3RemoveTagUnnecessaryByWalk {
        ...
        Files.walk(Path.of(args[0])).forEach(file -> {
            try {
                if (file.getFileName().toString().matches(args[1])) {
                    exec5_2(file.toAbsolutePath().toString());
                    exec5_3(file.toAbsolutePath().toString());
                }
            } catch (Exception e) {
